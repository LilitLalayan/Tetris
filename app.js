...
for (let i = 1; i < allCubesCount; i++) {
  const clonedItem = item.cloneNode();
  clonedItem.dataset.coord = i;
  container.append(clonedItem);
}
let addItem = document.createElement("div");
addItem.classList.add("occupied");
for (let i = 0; i < columnCount; i++) {
  const clonedItem = addItem.cloneNode();
  container.append(clonedItem);
}
let allItems = Array.from(items);
const addItems = document.getElementsByClassName("occupied");
let arrayFromAddItems = Array.from(addItems);
let squares = allItems.concat(arrayFromAddItems);
